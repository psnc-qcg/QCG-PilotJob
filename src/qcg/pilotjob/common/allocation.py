...
class Allocation:
    ...
    def export(self):
        """Export allocation description to the dictionary that can be deserialized as
        AllocationDescription class.
        """
        return {
            'binding': self._binding,
            'cores': self._cores,
            'nodes': [{'name': node.node.name, 'cores': node.cores } for node in self._nodes]
        }

class AllocationDescription:

    def __init__(self, binding, cores, nodes):
